...
function manualReviewResultStatusClass(status) {
  if (status === 'blocked') return 'pill risk';
  if (status === 'needs_follow_up' || status === 'in_progress') return 'pill warn';
  return 'pill';
}

function setOpenDataManualReviewResultKpi(payload) {
  const items = Array.isArray(payload?.results) ? payload.results : [];
  setText('[data-open-manual-review-result="total"]', String(payload?.total_count ?? 0));
  setText(
    '[data-open-manual-review-result="not-started"]',
    String(payload?.result_status_counts?.not_started ?? 0),
  );
  setText(
    '[data-open-manual-review-result="not-decided"]',
    String(payload?.reviewer_decision_counts?.not_decided ?? 0),
  );
  setText(
    '[data-open-manual-review-result="follow-up"]',
    String(items.filter(item => item.follow_up_required === true).length),
  );
  setText(
    '[data-open-manual-review-result="engineering"]',
    String(payload?.engineering_review_allowed_count ?? 0),
  );
  const crawlerDisabled = items.filter(item => item.crawler_execution_allowed === false).length;
  setText('[data-open-manual-review-result="crawler-disabled"]', String(crawlerDisabled));
  setText('[data-render="open-data-manual-review-result-updated"]', formatUpdatedText(payload?.generated_at));
}

function renderOpenDataManualReviewResultNote(payload) {
  const node = document.querySelector('[data-render="open-data-manual-review-result-note"]');
  if (!node) return;
  node.textContent = '';
  const lines = [
    `public_use_status：${valueOrPending(payload?.public_use_status)}`,
    '這是人工審核結果輸入表，不是 crawler。',
    'manual review required / no auto publish / no live crawler。',
    '不對 source_url 發出網路請求，不抓個資，不抓私人陳情全文。',
    `engineering_review_allowed_count：${valueOrPending(payload?.engineering_review_allowed_count)}`,
    `crawler_execution_allowed：${payload?.crawler_execution_allowed === false ? 'false' : valueOrPending(payload?.crawler_execution_allowed)}`,
...
function filterOpenDataManualReviewResults(items) {
  const keyword = normalizedValue(document.querySelector('#openDataManualReviewResultSearch')?.value);
  const reviewDay = document.querySelector('#openDataManualReviewResultDay')?.value || 'all';
  const reviewBatch = document.querySelector('#openDataManualReviewResultBatch')?.value || 'all';
  const resultStatus = document.querySelector('#openDataManualReviewResultStatus')?.value || 'all';
  const decision = document.querySelector('#openDataManualReviewResultDecision')?.value || 'all';
  const topicGroup = document.querySelector('#openDataManualReviewResultTopicGroup')?.value || 'all';

  return items.filter(item => {
    if (reviewDay !== 'all' && item.review_day !== reviewDay) return false;
    if (reviewBatch !== 'all' && item.review_batch !== reviewBatch) return false;
    if (resultStatus !== 'all' && item.result_status !== resultStatus) return false;
    if (decision !== 'all' && item.reviewer_decision !== decision) return false;
    if (topicGroup !== 'all' && item.topic_group !== topicGroup) return false;
    if (!keyword) return true;
...
function renderOpenDataManualReviewResultTable(items, totalCount) {
  const tbody = document.querySelector('[data-render="open-data-manual-review-result-table"]');
  const summary = document.querySelector('[data-render="open-data-manual-review-result-summary"]');
  if (!tbody) return;

  if (summary) {
    summary.textContent = `共 ${totalCount} 筆 result template，篩選後 ${items.length} 筆。`;
  }
  ...
  if (!items.length) {
    ...
    cell.colSpan = 15;
    cell.className = 'empty';
    cell.textContent = '目前沒有符合條件的人工審核結果模板。';
    row.appendChild(cell);
    tbody.appendChild(row);
    return;
  }
  ...
  items.forEach(item => {
    ...
    const statusCell = document.createElement('td');
    const statusPill = document.createElement('span');
    statusPill.className = manualReviewResultStatusClass(item.result_status);
    statusPill.textContent = valueOrPending(item.result_status);
    statusCell.appendChild(statusPill);
    ...
    const titleCell = createCell(item.title);
    const ownerCell = createCell(item.source_owner);
    const sourceOpenedCell = createCell(item.source_opened_result);
    const officialSourceCell = createCell(item.official_source_result);
    const licenseCell = createCell(item.license_terms_result);
    const formatCell = createCell(item.format_result);
    const personalCell = createCell(item.personal_data_result);
    const complaintCell = createCell(item.private_complaint_result);
...
function setupOpenDataManualReviewResults(payload) {
  const items = Array.isArray(payload?.results) ? payload.results : [];
  setOpenDataManualReviewResultKpi(payload);
  renderOpenDataManualReviewResultNote(payload);

  const render = () => {
    const filtered = filterOpenDataManualReviewResults(items).sort((a, b) => {
      if (a.review_day !== b.review_day) {
        return String(a.review_day || '').localeCompare(String(b.review_day || ''));
      }
      return String(a.title || '').localeCompare(String(b.title || ''), 'zh-Hant');
    });
    renderOpenDataManualReviewResultTable(filtered, items.length);
  };
  const searchInput = document.querySelector('#openDataManualReviewResultSearch');
  const daySelect = document.querySelector('#openDataManualReviewResultDay');
  const batchSelect = document.querySelector('#openDataManualReviewResultBatch');
  const statusSelect = document.querySelector('#openDataManualReviewResultStatus');
  const decisionSelect = document.querySelector('#openDataManualReviewResultDecision');
  const groupSelect = document.querySelector('#openDataManualReviewResultTopicGroup');
  if (searchInput) searchInput.addEventListener('input', render);
  if (daySelect) daySelect.addEventListener('change', render);
  if (batchSelect) batchSelect.addEventListener('change', render);
  if (statusSelect) statusSelect.addEventListener('change', render);
  if (decisionSelect) decisionSelect.addEventListener('change', render);
  if (groupSelect) groupSelect.addEventListener('change', render);
  render();
}

function setOpenDataManualReviewSopKpi(payload) {
  const reviewDays = payload?.review_days || {};
  const dailyItems = [];
  const dailyBatches = Array.isArray(payload?.sop?.daily_batches) ? payload.sop.daily_batches : [];
  dailyBatches.forEach(batch => {
    if (Array.isArray(batch?.items)) dailyItems.push(...batch.items);
  });
  setText('[data-open-manual-review-sop="total"]', String(payload?.total_tasks ?? 0));
  setText('[data-open-manual-review-sop="day_1"]', String(reviewDays.day_1 ?? 0));
  setText('[data-open-manual-review-sop="day_2"]', String(reviewDays.day_2 ?? 0));
  setText('[data-open-manual-review-sop="day_3"]', String(reviewDays.day_3 ?? 0));
  setText('[data-open-manual-review-sop="minutes"]', String(payload?.estimated_total_minutes ?? 0));
  setText(
    '[data-open-manual-review-sop="crawler-disabled"]',
    String(dailyItems.filter(item => item.crawler_execution_allowed === false).length),
  );
  setText('[data-render="open-data-manual-review-sop-updated"]', formatUpdatedText(payload?.generated_at));
}

function renderOpenDataManualReviewSopNote(payload) {
  const node = document.querySelector('[data-render="open-data-manual-review-sop-note"]');
  if (!node) return;
  node.textContent = '';
  const lines = [
    `public_use_status：${valueOrPending(payload?.public_use_status)}`,
    '這是人工審核 SOP，不是 crawler。',
    'manual review required / no auto publish / no live crawler。',
    '不對 source_url 發出網路請求，不抓個資，不抓私人陳情全文。',
    `engineering_review_allowed_count：${valueOrPending(payload?.engineering_review_allowed_count)}`,
    `crawler_execution_allowed：${payload?.crawler_execution_allowed === false ? 'false' : valueOrPending(payload?.crawler_execution_allowed)}`,
...
function renderOpenDataManualReviewSopDays(batches) {
  const node = document.querySelector('[data-render="open-data-manual-review-sop-days"]');
  const summary = document.querySelector('[data-render="open-data-manual-review-sop-summary"]');
  if (!node) return;

  if (summary) {
    summary.textContent = `共有 ${batches.length} 個批次，依 day_1、day_2、day_3 安排人工審核。`;
  }
  ...
  batches.forEach(batch => {
    ...
    title.textContent = valueOrPending(batch.review_day);
    const desc = document.createElement('p');
    desc.textContent = `${valueOrPending(batch.task_count)} 筆任務，預估 ${valueOrPending(batch.estimated_minutes_total)} 分鐘，批次：${valueOrPending(batch.items?.[0]?.review_batch || '待補')}`;
    card.append(title, desc);
    node.appendChild(card);
  });
}

function renderOpenDataManualReviewSopSteps(sop) {
  const node = document.querySelector('[data-render="open-data-manual-review-sop-steps"]');
  if (!node) return;
  node.textContent = '';

  const sections = [
    ['開始前檢查', sop?.pre_review_checklist || []],
    ['逐筆審核步驟', sop?.per_source_review_steps || []],
    ['證據記錄步驟', sop?.evidence_recording_steps || []],
    ['結果模板填寫', sop?.result_template_fill_steps || []],
    ['安全檢查', sop?.safety_checklist || []],
    ['完成與交接', sop?.handoff_next_actions || []],
  ];

  sections.forEach(([titleText, items]) => {
    const card = document.createElement('article');
...
function renderOpenDataManualReviewSopTable(batches) {
  const tbody = document.querySelector('[data-render="open-data-manual-review-sop-table"]');
  if (!tbody) return;
  tbody.textContent = '';

  const items = [];
  batches.forEach(batch => {
    (batch.items || []).forEach(item => items.push({ ...item, review_day: batch.review_day }));
  });
  ...
  if (!items.length) {
    ...
    cell.colSpan = 9;
    cell.className = 'empty';
    cell.textContent = '目前沒有可顯示的人工審核 SOP 任務。';
    row.appendChild(cell);
    tbody.appendChild(row);
    return;
  }
  ...
  items.forEach(item => {
    ...
    const ownerCell = createCell(item.source_owner);
    const actionCell = createCell(item.recommended_next_action, 'compact');
    const statusCell = createCell(item.result_status);
    const decisionCell = createCell(item.reviewer_decision);
    const urlCell = document.createElement('td');
    const link = document.createElement('a');
    link.className = 'url-link';
...
function setupOpenDataManualReviewSop(payload) {
  const dailyBatches = Array.isArray(payload?.sop?.daily_batches) ? payload.sop.daily_batches : [];
  setOpenDataManualReviewSopKpi(payload);
  renderOpenDataManualReviewSopNote(payload);
  renderOpenDataManualReviewSopDays(dailyBatches);
  renderOpenDataManualReviewSopSteps(payload?.sop || {});
  renderOpenDataManualReviewSopTable(dailyBatches);
}

function setOpenDataManualReviewPacketsKpi(payload) {
  const reviewDays = payload?.review_days || {};
  const packets = Array.isArray(payload?.packets) ? payload.packets : [];
  const taskCards = packets.flatMap(packet => Array.isArray(packet?.task_cards) ? packet.task_cards : []);
  setText('[data-open-manual-review-packets="total"]', String(payload?.total_tasks ?? 0));
  setText('[data-open-manual-review-packets="packet-count"]', String(payload?.packet_count ?? 0));
  setText('[data-open-manual-review-packets="day_1"]', String(reviewDays.day_1 ?? 0));
  setText('[data-open-manual-review-packets="day_2"]', String(reviewDays.day_2 ?? 0));
  setText('[data-open-manual-review-packets="day_3"]', String(reviewDays.day_3 ?? 0));
  setText('[data-open-manual-review-packets="minutes"]', String(payload?.estimated_total_minutes ?? 0));
  setText(
    '[data-open-manual-review-packets="crawler-disabled"]',
    String(taskCards.filter(item => item.crawler_execution_allowed === false).length),
  );
  setText('[data-render="open-data-manual-review-packets-updated"]', formatUpdatedText(payload?.generated_at));
}

function renderOpenDataManualReviewPacketsNote(payload) {
  const node = document.querySelector('[data-render="open-data-manual-review-packets-note"]');
  if (!node) return;
  node.textContent = '';
  const lines = [
    `public_use_status：${valueOrPending(payload?.public_use_status)}`,
    '這是人工審核工作包，不是 crawler。',
    'manual review required / no auto publish / no live crawler。',
    '不對 source_url 發出網路請求，不抓個資，不抓私人陳情全文。',
    `engineering_review_allowed_count：${valueOrPending(payload?.engineering_review_allowed_count)}`,
    `crawler_execution_allowed：${payload?.crawler_execution_allowed === false ? 'false' : valueOrPending(payload?.crawler_execution_allowed)}`,
...
function renderOpenDataManualReviewPacketCards(packets) {
  const node = document.querySelector('[data-render="open-data-manual-review-packets-cards"]');
  const summary = document.querySelector('[data-render="open-data-manual-review-packets-summary"]');
  if (!node) return;
  if (summary) {
    summary.textContent = `共有 ${packets.length} 個工作包，依 day_1、day_2、day_3 安排人工審核。`;
  }
  node.textContent = '';
  packets.forEach(packet => {
    const card = document.createElement('article');
    card.className = 'card packet-card';
    const title = document.createElement('h3');
    title.textContent = valueOrPending(packet.review_day);
    const desc = document.createElement('p');
    desc.textContent = `${valueOrPending(packet.task_count)} 筆任務，預估 ${valueOrPending(packet.estimated_minutes_total)} 分鐘，批次：${valueOrPending(packet.review_batch)}`;
    card.append(title, desc);
    node.appendChild(card);
  });
}

function renderOpenDataManualReviewPacketsTable(packets) {
  const tbody = document.querySelector('[data-render="open-data-manual-review-packets-table"]');
  if (!tbody) return;
  tbody.textContent = '';
  const items = [];
  packets.forEach(packet => {
    (packet.task_cards || []).forEach(task => items.push({ ...task, review_day: packet.review_day }));
  });
  if (!items.length) {
    const row = document.createElement('tr');
    const cell = document.createElement('td');
    cell.colSpan = 9;
    cell.className = 'empty';
    cell.textContent = '目前沒有可顯示的人工審核工作包任務。';
    row.appendChild(cell);
    tbody.appendChild(row);
    return;
  }
  items.forEach(item => {
    ...
    link.textContent = valueOrPending(item.source_url);
    urlCell.appendChild(link);
    row.append(dayCell, batchCell, titleCell, topicCell, ownerCell, actionCell, statusCell, decisionCell, urlCell);
    tbody.appendChild(row);
  });
}

function setupOpenDataManualReviewPackets(payload) {
  const packets = Array.isArray(payload?.packets) ? payload.packets : [];
  setOpenDataManualReviewPacketsKpi(payload);
  renderOpenDataManualReviewPacketsNote(payload);
  renderOpenDataManualReviewPacketCards(packets);
...
function setOpenDataDay1SampleResultsKpi(payload) {
  const items = Array.isArray(payload?.samples) ? payload.samples : [];
  const sampleStatusCounts = payload?.sample_status_counts || {};
  setText('[data-open-day1-sample="total"]', String(payload?.total_count ?? 0));
  setText('[data-open-day1-sample="sample-only"]', payload?.sample_only === true ? 'true' : valueOrPending(payload?.sample_only));
  setText('[data-open-day1-sample="not-actual"]', payload?.not_actual_review_result === true ? 'true' : valueOrPending(payload?.not_actual_review_result));
  setText('[data-open-day1-sample="status"]', String(sampleStatusCounts.sample_not_actual_review ?? 0));
  setText(
    '[data-open-day1-sample="crawler-disabled"]',
    String(items.filter(item => item.crawler_execution_allowed === false).length),
  );
  setText('[data-open-day1-sample="engineering-count"]', String(payload?.engineering_review_allowed_count ?? 0));
  setText('[data-render="open-data-day1-sample-updated"]', formatUpdatedText(payload?.generated_at));
}

function renderOpenDataDay1SampleResultsNote(payload) {
  const node = document.querySelector('[data-render="open-data-day1-sample-note"]');
  if (!node) return;
  node.textContent = '';
  const lines = [
    `public_use_status：${valueOrPending(payload?.public_use_status)}`,
    '這是 sample，不是實際審核結果，也不是 crawler。',
    'sample only / not actual review result / no auto publish / no live crawler。',
    '不對 source_url 發出網路請求，不抓個資，不抓私人陳情全文。',
    `engineering_review_allowed_count：${valueOrPending(payload?.engineering_review_allowed_count)}`,
    `crawler_execution_allowed：${payload?.crawler_execution_allowed === false ? 'false' : valueOrPending(payload?.crawler_execution_allowed)}`,
  ];
...
function renderOpenDataDay1SampleResultsTable(items, payload) {
  const tbody = document.querySelector('[data-render="open-data-day1-sample-table"]');
  const summary = document.querySelector('[data-render="open-data-day1-sample-summary"]');
  if (!tbody) return;
  if (summary) {
    summary.textContent = `顯示 ${items.length} 筆 ${valueOrPending(payload?.review_day)} 的 sample rows。`;
  }
  tbody.textContent = '';
  if (!items.length) {
    const row = document.createElement('tr');
    ...
    cell.colSpan = 10;
    cell.className = 'empty';
    cell.textContent = '目前沒有可顯示的 Day 1 sample rows。';
    row.appendChild(cell);
    tbody.appendChild(row);
    return;
  }
  items.forEach(item => {
    const row = document.createElement('tr');
    const statusCell = createCell(item.sample_status);
    const titleCell = createCell(item.title);
    const topicCell = createCell(item.topic_group);
    const ownerCell = createCell(item.source_owner);
    const decisionCell = createCell(item.sample_reviewer_decision);
    const actionCell = createCell(item.sample_recommended_next_action, 'compact');
    const followUpCell = createCell(item.sample_follow_up_required === true ? 'true' : 'false');
    const crawlerCell = createCell(item.crawler_execution_allowed === false ? 'false' : valueOrPending(item.crawler_execution_allowed));
    const engineeringCell = createCell(item.engineering_review_allowed === false ? 'false' : valueOrPending(item.engineering_review_allowed));
    const urlCell = document.createElement('td');
    const link = document.createElement('a');
...
function setupOpenDataDay1SampleResults(payload) {
  const items = Array.isArray(payload?.samples) ? payload.samples : [];
  setOpenDataDay1SampleResultsKpi(payload);
  renderOpenDataDay1SampleResultsNote(payload);
...
async function bootSitePages() {
  ...
  if (page === 'open-data-manual-review-results') {
    const payload = await loadJson('./data/open_data_manual_review_result_template.json', {
      generated_at: '',
    ...
    setupOpenDataManualReviewResults(payload);
  }
  if (page === 'open-data-manual-review-sop') {
    const payload = await loadJson('./data/open_data_manual_review_sop.json', {
      generated_at: '',
    ...
    setupOpenDataManualReviewSop(payload);
  }
  if (page === 'open-data-manual-review-packets') {
    const payload = await loadJson('./data/open_data_manual_review_execution_packets.json', {
      generated_at: '',
  ...
  if (page === 'open-data-day1-sample-results') {
    const payload = await loadJson('./data/open_data_day1_sample_manual_review_results.json', {
      generated_at: '',
