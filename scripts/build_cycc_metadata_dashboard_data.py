...
def build_minutes_item(index: int, row: dict[str, str]) -> dict[str, Any]:
    ...
    return {
        ...
        "notes": "internal metadata / manual review required / no auto publish",
    }


def build_video_item(index: int, row: dict[str, str]) -> dict[str, Any]:
    meeting_date = clean_text(row.get("meeting_date"))
    video_url = clean_text(row.get("video_url"))
    return {
        "record_id": f"cycc-video-{index:03d}",
        "source_type": "question_videos",
        "title": clean_text(row.get("video_title")) or "待補標題",
        "date": meeting_date,
...
def build_payload(source_type: str, items: list[dict[str, Any]], crawled_at: str) -> dict[str, Any]:
    return {
        "generated_at": now_taipei(),
        "latest_crawl_at": crawled_at,
...
def build_dashboard_metadata() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    crawled_at = to_taipei_iso(report.get("crawled_at", ""))
    minutes_rows = read_csv_rows(MINUTES_CSV)
    videos_rows = read_csv_rows(VIDEOS_CSV)
    minutes_items = [build_minutes_item(index, row) for index, row in enumerate(minutes_rows, start=1)]
    video_items = [build_video_item(index, row) for index, row in enumerate(videos_rows, start=1)]
    return (
        build_payload("minutes", minutes_items, crawled_at),
        build_payload("question_videos", video_items, crawled_at),
    )


def write_dashboard_metadata() -> tuple[dict[str, Any], dict[str, Any]]:
    DASHBOARD_DATA_DIR.mkdir(parents=True, exist_ok=True)
    minutes_payload, videos_payload = build_dashboard_metadata()
    MINUTES_OUTPUT.write_text(json.dumps(minutes_payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    VIDEOS_OUTPUT.write_text(json.dumps(videos_payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return minutes_payload, videos_payload


if __name__ == "__main__":
    minutes_payload, videos_payload = write_dashboard_metadata()
    print(
        f"Wrote {minutes_payload['total_count']} minutes records and "
        f"{videos_payload['total_count']} question video records"
    )
