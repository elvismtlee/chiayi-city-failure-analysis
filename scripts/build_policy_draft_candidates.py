...
def load_source_records() -> list[dict[str, Any]]:
    if WEEKLY_SUMMARY_PATH.exists():
        summary = load_json(WEEKLY_SUMMARY_PATH)
        return summary.get("top_issues", [])
...
def problem_statement(item: dict[str, Any]) -> str:
    summary = item.get("summary") or item.get("issue_summary") or item.get("issue_title", "")
...
def build_policy_draft(item: dict[str, Any]) -> dict[str, Any]:
    source_candidate_id = item["source_candidate_id"] if "source_candidate_id" in item else item["candidate_id"]
...
def validate_policy_draft(item: dict[str, Any]) -> None:
    ...
    if "人工確認" not in item["risk_notes"]:
        raise ValueError("risk_notes must remind reviewers to confirm manually.")


def build_policy_drafts(records: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    source_records = records if records is not None else load_source_records()
    return [build_policy_draft(item) for item in source_records]
