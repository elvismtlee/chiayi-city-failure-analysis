...
def build_needs_review(candidates: list[dict[str, Any]]) -> list[dict[str, str]]:
    return [
        {
            "source_candidate_id": item["candidate_id"],
            "issue_title": item["issue_title"],
...
def build_suggested_policy_topics(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    topics = []
    for item in candidates:
...
def build_weekly_summary(candidates: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    departments = [item.get("department", "") for item in source_candidates]
    keywords = [
        keyword
        for item in source_candidates
        for keyword in (item.get("issue_keywords") or [])
    ]
    summary = {
        ...
        "department_summary": count_summary(departments),
        "keyword_summary": count_summary(keywords),
        "top_issues": build_top_issues(source_candidates),
        "needs_review": build_needs_review(source_candidates),
        "suggested_policy_topics": build_suggested_policy_topics(source_candidates),
