...
SCHEMA = ROOT / "config" / "open_data_day1_manual_review_form_draft_schema.yml"
...
def test_open_data_day1_manual_review_form_draft_schema_exists_and_flags_are_safe() -> None:
    data = load_schema()
    assert SCHEMA.exists()
    assert data["public_use_status"] == "internal_day1_manual_review_form_draft_schema"
...
def test_open_data_day1_manual_review_form_draft_schema_contains_required_fields() -> None:
    data = load_schema()
    required_fields = set(data["required_fields"])
    assert "form_id" in required_fields
    assert "reviewer_fields" in required_fields
    assert "source_identity_section" in required_fields
    assert "risk_review_section" in required_fields
