...
SCHEMA = ROOT / "config" / "open_data_day1_manual_review_operation_board_schema.yml"
...
def test_open_data_day1_manual_review_operation_board_schema_exists_and_flags_are_safe() -> None:
    data = load_schema()
    assert SCHEMA.exists()
    assert data["public_use_status"] == "internal_day1_manual_review_operation_board_schema"
...
def test_open_data_day1_manual_review_operation_board_schema_contains_required_fields() -> None:
    data = load_schema()
    required_fields = set(data["required_fields"])
    assert "board_id" in required_fields
    assert "task_cards" in required_fields
    assert "workflow_steps" in required_fields
